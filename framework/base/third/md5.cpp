...
void MD5::Update(const char * pBuffer, int iLen)
{
	if ( !finished_ ) {
		md5_update(&context_, (uint8*)pBuffer, iLen);
	}
}

const char * MD5::GetCheckSum()
{
	if ( !finished_ ) {
		unsigned char md5bytes[16];
		md5_finish(&context_, md5bytes);

		for(size_t i=0; i<sizeof(md5bytes); i++) {
			sprintf(md5str_ + i * 2, "%02x", md5bytes[i]);
		}
